...
def validate_acompañante_tipo(self) -> bool:
    """
    ...
    if not hasattr(self, 'tipo'):
        raise ValueError(f"Acompañante '{self.nombre}' must have 'tipo' property")
    
    if not self.tipo or str(self.tipo).strip() == '':
        raise ValueError(f"Acompañante '{self.nombre}' must have non-empty 'tipo'")
    
    return True


MethodRegistry.register_validator('Acompañante', validate_acompañante_tipo)